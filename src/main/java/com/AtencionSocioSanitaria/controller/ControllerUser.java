	...
	//Esto tiene que estar en una carpeta a la misma altura que la aplicación principal
		...
		public String listaUsuario(Model model, @RequestParam(defaultValue = "1", required = false) String page) {
			...
			try {
				...
				Page<Usuario> listUsuarios = usuarioServices.getUsuariosPagination(pageNumber, 25);
				model.addAttribute("list", listUsuarios);
				model.addAttribute("currentPage", pageNumber);
				model.addAttribute("maxPage", usuarioServices.getMaxPages(25));
	
			} catch (Exception e) {
				pageNumber = 1;
				int maxPage = usuarioServices.getMaxPages(25);
				Page<Usuario> listUsuarios = usuarioServices.getUsuariosPagination(pageNumber, maxPage);
				model.addAttribute("list", listUsuarios);
				model.addAttribute("currentPage", pageNumber);
		...
		@PostMapping("/usuario")
		public String filterAndListUser(Model model, 
		    ...
		    List<Usuario> usuariosFiltrados = listUsuarios.stream()
		        .filter(usuario ->
		            usuario.getNombre().toLowerCase().contains(dato.toLowerCase()) ||
		            usuario.getApellido().toLowerCase().contains(dato.toLowerCase()) ||
		            usuario.getPhones().stream().anyMatch(telefono -> telefono.getTelefono().contains(dato)) // Filtro por teléfono
		        )
		    ...
		    if (!orden.isEmpty()) {
		        switch (orden) {
		            ...
		            case "localidad": // Ordenar por localidad de forma alfabética
		            	usuariosFiltrados.sort(Comparator.comparing(usuario -> usuario.getLocalidadId().getName().toLowerCase()));
		                break;
		...
		public String addCharacterPost (Model model, @ModelAttribute Usuario u, 
				@RequestParam("telefono") String phoneNumber, @RequestParam("description") String phoneDesc) {
